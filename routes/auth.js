...
const { refreshToLongLivedToken } = require('../src/services/instagramService');
...
router.get('/callback', async (req, res) => {
    ...
    try {
        ...
        const { id: igUserId, name: fullName } = userRes.data;
        const encrypted = encrypt(longToken);
        const userQuery = await db.query('INSERT INTO Users (ig_user_id, full_name, access_token) VALUES ($1, $2, $3) ON CONFLICT (ig_user_id) DO UPDATE SET access_token = $3, full_name = $2 RETURNING id', [igUserId, fullName, encrypted]);
        const token = jwt.sign({ userId: userQuery.rows[0].id, igUserId }, process.env.JWT_SECRET, { expiresIn: '15m' });
