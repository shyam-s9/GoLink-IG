...
const worker = new Worker('messageQueue', async job => {
    ...
    try {
        ...
        if (name === 'process-dm') {
            ...
            const privateDmDelay = Math.floor(Math.random() * (max - min) + min) + jitter;
            const customLink = generateGoLink(automationId, followerIgId);
            await messageQueue.add('send-private-dm', { creatorIgId, followerIgId, message: `Here is your requested link: ${customLink}`, accessToken: decryptedToken, automationId }, { delay: privateDmDelay });
        }
